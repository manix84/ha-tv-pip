...
Stage 7 is complete in `0.24.0`. It adds `stream_type: auto`, `stream_type: hls`, `stream_type: mjpeg`, and `stream_type: snapshot` to `ha_tv_pip.show_camera`. Automatic mode prefers HLS, tries MJPEG when HLS URL resolution fails, falls back to snapshot when stream URL resolution still fails, and keeps the snapshot preview visible if receiver playback fails after a video URL is accepted.
