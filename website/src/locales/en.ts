...
export const en = {
    ...
    currentStatus: {
      body: [
        ...
        "Post-1.0 compatibility work is underway. The Home Assistant integration can test HLS, MJPEG, and snapshot availability for a camera, show the latest recommendation on the receiver device, save that recommendation as per-camera defaults, store the latest command result without stream URLs, and report whether the installed receiver is current, degraded, legacy, or incompatible.",
      ],
